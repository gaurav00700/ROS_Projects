...
class InformedRRTStar:
    ...
    def informed_rrt_star_search(self, animation=True):
        ...
        i=0
        i_ellipse=0
        while path_count <= 5 and i_ellipse ==100 :
...
def main(start=None,goal_points=None, maxIter=None, obs_points=None,):
    ...
    obs_points = np.genfromtxt('obs_points.txt', delimiter=',')
    obstacleList=[]
    ...
    x_min=min([point[0] for point in obs_points])
    y_min=min([point[1] for point in obs_points])
    x_max=max([point[0] for point in obs_points])
    y_max=max([point[1] for point in obs_points])
    goal_pnt_list=[]
    goal_len=[]
    path=None
    # START PARAMETERS
    ...
    expandDis=0.3 #min lenght of step
    ...
    for goal in goal_points:
        ...
        goal_pnt_list.append(path)
        goal_len.append(path_len)
    
    #return shortest path
    index_min=goal_len.index(min(goal_len)) #index of shortest path
    goal_points=goal_pnt_list[index_min] #shortest path
    
    #save best path points
    print('----------Best Goal length and path points:-----------')
    print(goal_points)
    print("Path_points saved in current directory")
    np.savetxt('path_points.txt', goal_points, delimiter=',', fmt='%f', comments='') 
    
    return goal_points
    
if __name__ == '__main__':
    main(points=None,start=None,goal=None, maxIter=None)
